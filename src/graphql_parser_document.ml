...
let parse_directive parser = Result_ext.(
    ...
    |> flat_map (fun { span = at_start, _; _ } -> expect_name parser |> map (make_t2 at_start))
...
let parse_directives parser = match peek parser with
  | { item = Graphql_lexer.At; _ } -> 
    let rec scanner acc = match peek parser with
      | { item = Graphql_lexer.At; _ } -> begin match parse_directive parser with
          | Error e -> Error e
...
let rec parse_type parser = Result_ext.(
    ...
    |> flat_map (function
        | Some { span = start_pos, _; _ } ->
          ...
          |> flat_map (fun (inner_type, { span = _, (_ as bracket_end_pos); _ }) ->
...
and parse_optional_selection_set parser = match peek parser with
  | { item = Graphql_lexer.Curly_open; _ } -> Result_ext.map (fun x -> Some x) (parse_selection_set parser)
  | _ -> Ok None
...
and parse_field parser = Result_ext.(
    let parse_rest alias name =
      ...
      |> map (fun (arguments, directives, selection_set) ->
          start_end
            (match alias with | Some { span = start_pos, _; _ } -> start_pos | None -> name |> start_pos)
            (match selection_set with | Some { span = _, end_pos; _ } -> end_pos | None ->
              match last directives with | Some { span = _, end_pos; _ } -> end_pos | None ->
              match arguments with | Some { span = _, end_pos; _ } -> end_pos | None -> name |> end_pos)
            {
              fd_alias = alias;
              fd_name = name;
...
and parse_fragment parser = Result_ext.(
    ...
    | Error e -> Error e
    | Ok { span = start_pos, _; _ } -> match peek parser with
...
let parse_operation_type parser = match next parser with
  | Error e -> Error e
  | Ok ({ item = Graphql_lexer.Name "query"; _ } as span) -> Ok (replace span Query)
  | Ok ({ item = Graphql_lexer.Name "mutation"; _ } as span) -> Ok (replace span Mutation)
  | Ok ({ item = Graphql_lexer.Name "subscription"; _ } as span) -> Ok (replace span Subscription)
  | Ok span -> Error (map (fun t -> Unexpected_token t) span)

let parse_operation_definition parser = Result_ext.(
    match peek parser with
    | { item = Graphql_lexer.Curly_open; _ } -> begin match parse_selection_set parser with
        | Error e -> Error e
    ...
    | { span = start_pos, _; _ } -> 
      ...
      | Ok operation_type -> match peek parser with
        | { item = Graphql_lexer.Name _; _ } -> begin match expect_name parser with
            | Error e -> Error e
...
let parse_fragment_definition parser = Result_ext.(
    ...
    |> flat_map (fun (start_pos, name) -> match name with
        | { item = "on"; _ } as span -> Error (Source_pos.replace span (Unexpected_token (Graphql_lexer.Name "on")))
        | _ -> Ok(start_pos, name))
