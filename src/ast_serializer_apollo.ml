...
let ser_optional_list serializer = function
  | None | Some { item = []; _ } -> [%expr Js.Json.array([||])]
...
let rec ser_type = function
  | { item = Tr_named name; _ } -> [%expr Js.Json.object_(Js.Dict.fromArray([|
      ("kind", Js.Json.string("NamedType"));
      ("name", [%e ser_name name]);
    |]))]
  | { item = Tr_list inner; _ } -> [%expr Js.Json.object_(Js.Dict.fromArray([|
      ("kind", Js.Json.string("ListType"));
      ("type", [%e ser_type inner]);
    |]))]
  | { item = Tr_non_null_named name; _ } -> [%expr Js.Json.object_(Js.Dict.fromArray([|
      ...
      |])))
    |]))]
  | { item = Tr_non_null_list inner; _ } -> [%expr Js.Json.object_(Js.Dict.fromArray([|
      ("kind", Js.Json.string("NonNullType"));
      ("type", Js.Json.object_(Js.Dict.fromArray([|
        ("kind", Js.Json.string("ListType"));
        ("type", [%e ser_type inner]);
...
and ser_selection = function
  | Field { item; _ } -> [%expr Js.Json.object_(Js.Dict.fromArray([|
    ...
    |]))]
  | FragmentSpread { item; _ } -> [%expr Js.Json.object_(Js.Dict.fromArray([|
      ("kind", Js.Json.string("FragmentSpread"));
      ("name", [%e ser_name item.fs_name]);
      ("directives", [%e ser_directives item.fs_directives]);
    |]))]
  | InlineFragment { item; _ } -> [%expr Js.Json.object_(Js.Dict.fromArray([|
...
let ser_definition = function
  | Operation { item; _ } -> [%expr Js.Json.object_(Js.Dict.fromArray([|
    ...
    |]))]
  | Fragment { item; _ } -> [%expr Js.Json.object_(Js.Dict.fromArray([|
